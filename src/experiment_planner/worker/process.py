...
def _run(connection, snapshot, request):
    try:
        from experiment_planner.engine.planner import generate
        connection.send({"result": generate(snapshot, request)})
...
class CalculationTask:
    ...
    def __init__(self, project, request):
        ...
        self.process = context.Process(target=_run, args=(child, snapshot, request), daemon=True)
