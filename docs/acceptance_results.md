...
- Ubuntu / Python 3.12：核心、存储、模型、后台取消、本地图形与 Qt 测试最终全量 **65 项通过**（0 失败、0 错误，23.62 秒）。待做点多轮回归已包含。机器可读证据：[linux-validation.json](evidence/linux-validation.json)。
...
![中文最小桌面原型（合成数据）](images/prototype.png)
